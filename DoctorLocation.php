            ...
            // Attempt to execute the prepared statement
                    if($stmt1 ->execute())
                    {
                        ...
                        if($stmt2 = $conn->prepare($sql2))
                        {
                            ...
                            if($stmt2 ->execute())
                            {
                                $query2 = "SELECT Doc_ID FROM doctors WHERE Location_ID=$location_id and First_Name='$fname' and Middle_Name='$mname' and Last_Name='$sname' and Specialization='$spec' and Qualification='$qual';";
                                $res2=mysqli_query($conn, $query2);
                                    foreach ($res2 as $loc2)
                                        {
                                            $doc_id=$loc2['Doc_ID'];
                                            echo $doc_id;
                                        }
                            ...
                            $sql3= "INSERT INTO doc_mail (Doc_ID,Email) VALUES(?,?);";
                            if($stmt3=$conn->prepare($sql3)){
                            // Bind variables to the prepared statement as parameters
                            $stmt3->bind_param("ss",$param_Doc_Id,$param_Email);
                            #echo "    nanansndn    ";
                            #echo $doc_id."    ";
                            // Set parameters
                            $param_Doc_Id=$doc_id;
                            $param_Email=$email;
                            echo $param_Doc_Id.$param_Email;
                            // Attempt to execute the prepared statement
                            if($stmt3->execute())
                            {
                                #echo "working 3";
                            ...
                            $sql4= "INSERT INTO doc_phone (Doc_ID,Phone_No) VALUES(?,?);";
                            if($stmt4=$conn->prepare($sql4)){
                            // Bind variables to the prepared statement as parameters
                            $stmt4->bind_param("ss",$param_Doc_Id,$param_Email);
                            #echo "    nanansndn    ";
                            #echo $doc_id."    ";
                            // Set parameters
                            $param_Doc_Id=$doc_id;
